...
static int select2;                  // The currently selected item
...
char* menu(void)
{
    ...
    while (!menu_done) {
        ...
        if (selection_offset != 0) {
            if (!has_moved_selection) select2 += selection_offset;
        ...
        if (select2 > item_count-1) select2 = item_count-1;
        ...
        for (int i = 0; i < item_count; i++) {
            if (select2 == i) yselect_target = ycur;
        ...
        if (current_screen == SCREEN_MINIGAME) {
            ...
            Minigame *cur = &global_minigame_list[sorted_indices[select2]];
